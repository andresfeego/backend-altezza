...
function configureAttendance(modules, fallback) {
  if (!Array.isArray(modules) || modules.filter(m => m.type === 'dresscode').length !== 1) throw new Error('Se requiere un módulo de vestuario único.');
  if (fallback?.type !== 'attendance_confirm' || (!fallback.config?.title || !fallback.config?.helperText) || modules.filter(m => m.type === 'attendance_confirm').length > 1) throw new Error('Configuración de asistencia ambigua.');
  const current = modules.find(m => m.type === 'attendance_confirm');
  const next = modules.filter(m => m.type !== 'attendance_confirm').sort((a, b) => a.order - b.order);
  const attendance = { ...fallback, ...current, enabled: true, config: { ...current?.config, ...fallback.config } };
  next.splice(next.findIndex(m => m.type === 'dresscode') + 1, 0, attendance);
...
async function run() {
  ...
  try {
    ...
    const fallback = JSON.parse(fs.readFileSync(path.join(__dirname, 'modules.json'), 'utf8')).find(m => m.type === 'attendance_confirm');
    const after = configureAttendance(JSON.parse(before), fallback);
    if (JSON.stringify(JSON.parse(before)) !== JSON.stringify(after)) {
      const backup = path.join(os.tmpdir(), `bodlauser-before-attendance-${Date.now()}.json`);
    ...
    await db.commit();
    console.log(JSON.stringify({ event: 'bodlauser', attendance: after.find(m => m.type === 'attendance_confirm') }));
  } catch (error) { await db.rollback(); throw error; }
  finally { await db.end(); }
}
module.exports = { configureAttendance, run };
