...
function configureClosing(modules, fallback) {
  if (!Array.isArray(modules) || modules.filter(m => m.type === 'attendance_confirm').length !== 1) throw new Error('Se requiere un módulo de asistencia único.');
  if (fallback?.type !== 'closing_message' || (!fallback.config?.message || !fallback.config?.imageSrc) || modules.filter(m => m.type === 'closing_message').length > 1) throw new Error('Configuración de cierre ambigua.');
  const current = modules.find(m => m.type === 'closing_message');
  const next = modules.filter(m => m.type !== 'closing_message').sort((a, b) => a.order - b.order);
  const closing = { ...fallback, ...current, enabled: true, config: { ...current?.config, ...fallback.config } };
  next.splice(next.findIndex(m => m.type === 'attendance_confirm') + 1, 0, closing);
...
async function run() {
  ...
  try {
    ...
    const fallback = JSON.parse(fs.readFileSync(path.join(__dirname, 'modules.json'), 'utf8')).find(m => m.type === 'closing_message');
    const after = configureClosing(JSON.parse(before), fallback);
    if (JSON.stringify(JSON.parse(before)) !== JSON.stringify(after)) {
      const backup = path.join(os.tmpdir(), `bodlauser-before-closing-${Date.now()}.json`);
    ...
    await db.commit();
    console.log(JSON.stringify({ event: 'bodlauser', closing: after.find(m => m.type === 'closing_message') }));
  } catch (error) { await db.rollback(); throw error; }
  finally { await db.end(); }
}
module.exports = { configureClosing, run };
