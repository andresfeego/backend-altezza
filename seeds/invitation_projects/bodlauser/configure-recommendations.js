...
function configureRecommendations(modules, fallback) {
  if (!Array.isArray(modules) || modules.filter(m => m.type === 'gift_envelopes').length !== 1) throw new Error('Se requiere un módulo de sobres único.');
  if (fallback?.type !== 'recommendations' || !fallback.config?.imageSrc || modules.filter(m => m.type === 'recommendations').length > 1) throw new Error('Configuración de recomendaciones ambigua.');
  const current = modules.find(m => m.type === 'recommendations');
  const next = modules.filter(m => m.type !== 'recommendations').sort((a, b) => a.order - b.order);
  const recommendation = { ...fallback, ...current, enabled: true, config: { ...current?.config, ...fallback.config } };
  next.splice(next.findIndex(m => m.type === 'gift_envelopes') + 1, 0, recommendation);
...
async function run() {
  ...
  try {
    ...
    const fallback = JSON.parse(fs.readFileSync(path.join(__dirname, 'modules.json'), 'utf8')).find(m => m.type === 'recommendations');
    const after = configureRecommendations(JSON.parse(before), fallback);
    if (JSON.stringify(JSON.parse(before)) !== JSON.stringify(after)) {
      const backup = path.join(os.tmpdir(), `bodlauser-before-recommendations-${Date.now()}.json`);
    ...
    await db.commit();
    console.log(JSON.stringify({ event: 'bodlauser', recommendation: after.find(m => m.type === 'recommendations') }));
  } catch (error) { await db.rollback(); throw error; }
  finally { await db.end(); }
}
module.exports = { configureRecommendations, run };
