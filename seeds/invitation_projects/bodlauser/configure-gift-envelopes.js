...
function configureGiftEnvelopes(modules, fallback) {
  if (!Array.isArray(modules) || modules.filter(m => m.type === 'event_details').length !== 1) throw new Error('Se requiere un módulo de detalles único.');
  if (fallback?.type !== 'gift_envelopes' || !fallback.config?.imageSrc || modules.filter(m => m.type === 'gift_envelopes').length > 1) throw new Error('Configuración de sobres ambigua.');
  const current = modules.find(m => m.type === 'gift_envelopes');
  const next = modules.filter(m => m.type !== 'gift_envelopes').sort((a, b) => a.order - b.order);
  const gifts = { ...fallback, ...current, enabled: true, config: { ...current?.config, ...fallback.config } };
  next.splice(next.findIndex(m => m.type === 'event_details') + 1, 0, gifts);
...
async function run() {
  ...
  try {
    ...
    const fallback = JSON.parse(fs.readFileSync(path.join(__dirname, 'modules.json'), 'utf8')).find(m => m.type === 'gift_envelopes');
    const after = configureGiftEnvelopes(JSON.parse(before), fallback);
    if (JSON.stringify(JSON.parse(before)) !== JSON.stringify(after)) {
      const backup = path.join(os.tmpdir(), `bodlauser-before-gift-envelopes-${Date.now()}.json`);
    ...
    await db.commit();
    console.log(JSON.stringify({ event: 'bodlauser', gift: after.find(m => m.type === 'gift_envelopes') }));
  } catch (error) { await db.rollback(); throw error; }
  finally { await db.end(); }
}
module.exports = { configureGiftEnvelopes, run };
