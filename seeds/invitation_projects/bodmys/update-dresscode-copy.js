/** Apply Garden Elegance copy only to the local Mayra/Samuel dresscode module. */
...
async function main() {
  ...
  const changes = [
    { type: 'dresscode', key: 'attireLabel', previous: 'Formal' },
    { type: 'dresscode', key: 'message', previous: '' },
  ];
  ...
  try {
    ...
    const backup = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'altezza-bodmys-dresscode-copy-')), 'modules-before.json');
