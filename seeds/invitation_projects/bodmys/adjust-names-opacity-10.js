/** Set names overlay opacity to 0.10 (90% transparency). */
...
function configureNamesBackground(modules, background) {
  ...
  return modules.map((module) => {
    ...
    if (![.2, .32].includes(current?.overlayOpacity) || current.imageSrc !== background.imageSrc) throw new Error('El fondo cambió; no se sobrescribe.');
    return { ...module, config: { ...module.config, sectionBackground: { ...current, overlayOpacity: background.overlayOpacity } } };
  });
}
...
async function main() {
  ...
  const background = { ...seed.find((m) => m.type === 'couple_names')?.config?.sectionBackground, overlayOpacity: .1 };
