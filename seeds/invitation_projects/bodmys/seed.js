...
async function seed() {
  ...
  const eventId = 'bodmys';
  const invitationId = 'mysprueba';
  try {
    ...
    if (existing.length && !['Mayra & Samuel', 'Mayra y Samuel'].includes(existing[0].nombre)) {
      throw new Error('bodmys pertenece a otro evento. No se modificó.');
    }
    ...
    const ceremony = await getPlace('Capilla del Cristo');
    const reception = await getPlace('Conjunto Villa Sarita 1');
    await db.query(`INSERT INTO evento
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)
      ON DUPLICATE KEY UPDATE nombre=VALUES(nombre), idTipoEvento=VALUES(idTipoEvento),
      fechaHoraCeremonia=VALUES(fechaHoraCeremonia), fechaHoraRecepcion=VALUES(fechaHoraRecepcion),
      fechaHoraLimiteConfirmar=VALUES(fechaHoraLimiteConfirmar), idLugarCeremonia=VALUES(idLugarCeremonia), idLugarRecepcion=VALUES(idLugarRecepcion)`,
      [eventId, 'Mayra & Samuel', types[0].id, '2026-11-28 15:00:00', '2026-11-28 16:30:00', '2026-11-19 00:00:00', ceremony, reception]);
    ...
    if (priorInvitation.length) {
      ...
      await db.query('INSERT INTO invitacion (id, label, mensaje_personalizado, enviada) VALUES (?, ?, ?, 0)', [invitationId, 'Prueba local · Mayra y Samuel', '']);
    ...
    const guestIds = [];
    for (const [index, name] of ['Invitado de prueba Oliva', 'Acompañante de prueba Oliva'].entries()) {
