/** Add the optional section background to this event's names module only. */
...
function configureNamesBackground(modules, background) {
  ...
  return modules.map((module) => {
    if (module.type !== 'couple_names' || Object.prototype.hasOwnProperty.call(module.config || {}, 'sectionBackground')) return module;
...
async function main() {
  ...
  const background = seed.find((m) => m.type === 'couple_names')?.config?.sectionBackground;
  ...
  try {
    ...
    const backup = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'altezza-bodmys-names-background-')), 'modules-before.json');
