/** Set names overlay opacity to 0.32 (68% transparency). */
...
function configureNamesBackground(modules, background) {
  ...
  return modules.map((module) => {
    ...
    if (current?.overlayOpacity !== .1 || current.imageSrc !== background.imageSrc) throw new Error('El fondo cambió; no se sobrescribe.');
...
async function main() {
  ...
  const background = { ...seed.find((m) => m.type === 'couple_names')?.config?.sectionBackground, overlayOpacity: .32 };
