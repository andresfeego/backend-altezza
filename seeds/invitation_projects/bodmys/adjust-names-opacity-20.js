/** Set names overlay opacity to 0.20 (80% transparency). */
...
function configureNamesBackground(modules, background) {
  ...
  return modules.map((module) => {
    ...
    if (current?.overlayOpacity !== .5 || current.imageSrc !== background.imageSrc) throw new Error('El fondo cambió; no se sobrescribe.');
...
async function main() {
  ...
  const background = { ...seed.find((m) => m.type === 'couple_names')?.config?.sectionBackground, overlayOpacity: .2 };
